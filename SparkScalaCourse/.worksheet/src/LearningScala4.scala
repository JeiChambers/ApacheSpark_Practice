...
val sum = numberList.reduce( (x: Int, y: Int) => x + y);System.out.println("""sum  : Int = """ + $show(sum ));$skip(58); 
val median = numberList.reduce((x: Int, y: Int) => y - x);System.out.println("""median  : Int = """ + $show(median ));$skip(13); 
...
val iHateThrees = numberList.filter(_ != 3);System.out.println("""iHateThrees  : List[Int] = """ + $show(iHateThrees ));$skip(55); 
val iHateFours = numberList.filter((x: Int) => x != 4);System.out.println("""iHateFours  : List[Int] = """ + $show(iHateFours ));$skip(41); 
val iHateSix = numberList.filter(_ != 6);System.out.println("""iHateSix  : List[Int] = """ + $show(iHateSix ));$skip(231); 
